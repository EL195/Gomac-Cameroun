...
changeme(){
	...
	if(this.course.engin=='Moto'){
		this.km=75;
	}
	if(this.course.engin=='Motocyclette'){
		this.km=400;
	}
	if(this.course.engin=='Personnel'){
		this.km=200;
	}
				...
				this.gps.geographie(this.arrive.getPosition().lng,this.arrive.getPosition().lat).then((resulta: NativeGeocoderResult[]) =>{
					...
					if(resulta[0]){
						...
						if(resulta[0].locality){
                          ...
                          if(this.course.engin=='Moto' && 200>this.course.prix){
                            this.course.prix=200;
                          }
                          if((this.course.engin=='Taxi'||this.course.engin=='Personnel') && 500>this.course.prix){
